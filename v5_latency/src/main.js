/**
 * Slackline Balance Simulator V5 — Main Entry Point (3-DOF + LQR + Latency)
 * 
 * LQR 최적 상태 피드백 제어기 + 센서/액추에이터 지연시간 시뮬레이션
 * 모드: 기본 LQR / 상태증강 LQR 선택 가능
 */
var SL = SL || {};
...
(function () {
    ...
    function init() {
        ...
        controller.setDelay(0, SL.Params.dt * 1000);
        ...
        renderer.draw(model.state, model, controller);

        console.log('Slackline Balance Simulator V5 (3-DOF + LQR + Latency) initialized');
    }
    ...
    function setupSliders() {
        ...
        // 지연시간 슬라이더
        const delaySlider = document.getElementById('slider-latency');
        const delayDisplay = document.getElementById('val-latency');
        if (delaySlider && delayDisplay) {
            delaySlider.min = 0; delaySlider.max = 100; delaySlider.step = 5;
            delaySlider.value = 0;
            delayDisplay.textContent = '0';
            delaySlider.addEventListener('input', () => {
                const val = parseFloat(delaySlider.value);
                controller.setDelay(val, SL.Params.dt * 1000);
                delayDisplay.textContent = val.toFixed(0);
                // 증강 모드면 게인 재계산
                if (controller.augmented) {
                    controller.computeAugmentedGains(SL.Params, val);
                }
            });
        }
    }
    ...
    function setupModeToggle() {
        ...
        const btnAug = document.getElementById('btn-mode-aug');
        const modeDesc = document.getElementById('mode-desc');

        function setMode(augmented) {
            controller.setAugmented(augmented, SL.Params);
            // 지연 재적용
            const delaySlider = document.getElementById('slider-latency');
            if (delaySlider) {
                controller.setDelay(parseFloat(delaySlider.value), SL.Params.dt * 1000);
            }
            if (augmented) {
                controller.computeAugmentedGains(SL.Params, controller.totalDelayMs);
            }
            ...
            if (augmented) {
                btnBasic.classList.remove('btn-primary');
                btnAug.classList.add('btn-primary');
                modeDesc.textContent = '상태증강: 지연을 반영한 최적 게인 재계산 (DARE)';
            } else {
                btnBasic.classList.add('btn-primary');
                btnAug.classList.remove('btn-primary');
                modeDesc.textContent = '기본: 고정 게인 + 지연된 상태 사용 (지연 미반영)';
            }
        }

        btnBasic.addEventListener('click', () => setMode(false));
        btnAug.addEventListener('click', () => setMode(true));
    }
