"""
V10 Physics Model -> PyBullet Verification
===========================================
V10의 사람 크기 모델(m1=30kg, m2=40kg)을 PyBullet에 그대로 이식.
직육면체 박스 2개 + 강체 줄 2개의 키네마틱 체인.

키네마틱 체인:
  base (pole_a_top, 고정)
    └─ rope_a (Y축 revolute) → φ
         └─ lower_body (Y축 revolute = ankle_a, +Y edge) → α
              ├─ upper_body (Y축 revolute = hip, top center) → θ [τ]
              └─ rope_b (Y축 revolute = ankle_b, -Y edge)
                   └─ P2P constraint → pole_b_top

조건: 줄=강체, 줄 질량≈0, Y축 revolute만 → V10 원호 모델과 동치
"""
import sys
import os
import time
...
BD = 0.25                      # body depth = foot spacing (m)

B_PHI = 2.0; B_ALPHA = 1.0; B_THETA = 0.5   # damping
...
ROPE_R = 0.012; ROPE_MASS = 0.001  # visual rope radius, near-zero mass
...
def main():
    ...
    rope_b_vis = p.createVisualShape(
        p.GEOM_CAPSULE, radius=ROPE_R,
        length=np.linalg.norm(rb_end),
        visualFramePosition=rb_mid.tolist(),
        visualFrameOrientation=capsule_quat(rb_end),
        rgbaColor=[1, .8, 0, 1])
    ...
    # --- Robot multibody (rope_a + lower_body + upper_body) ---
    # rope_b is a SEPARATE body to avoid Jacobian degeneracy
    # in the closed-loop constraint solver.
    ...
    robot_id = p.createMultiBody(
        ...
        linkMasses=[ROPE_MASS, M1, M2],
        linkCollisionShapeIndices=[-1, lower_col, upper_col],
        linkVisualShapeIndices=[rope_a_vis, lower_vis, upper_vis],

        linkPositions=[
            [0, 0, 0],           # rope_a joint at base (pole_a_top)
            ra_end.tolist(),     # ankle_a at rope_a endpoint
            [0, -BD/2, L1],     # hip at lower body top center
        ],
        linkOrientations=[[0,0,0,1]] * 3,

        linkInertialFramePositions=[
            [0, 0, 0],              # rope_a: CoM at joint
            [0, -BD/2, L1/2],       # lower: CoM at body center
            [0, 0, L2/2],           # upper: CoM at body center
        ],
    ...
    # --- Rope B as separate body ---
    # Origin at ankle_b world position (initial config)
    ankle_b_init = np.array([0, -BD/2, FOOT_Z])
    rope_b_id = p.createMultiBody(
        baseMass=ROPE_MASS,
        basePosition=ankle_b_init.tolist(),
        baseCollisionShapeIndex=-1,
        baseVisualShapeIndex=rope_b_vis,
    )

    # rope_b is visual-only: no constraints, no physics interaction.
    # It's repositioned each step to follow ankle_b → pole_b.
    ankle_b_in_lower = np.array([0, -BD/2, -L1/2])  # ankle_b in lower CoM frame
    pole_b_pos = np.array([0, -HALF_POLE, POLE_H])
    rb_dir0 = rb_end / np.linalg.norm(rb_end)  # initial rope_b direction (normalized)

    def update_rope_b_visual():
        ls = p.getLinkState(robot_id, ANKLE, computeForwardKinematics=True)
        com_pos = np.array(ls[0])  # lower body CoM world pos
        com_orn = ls[1]            # lower body CoM world orientation
        R = np.array(p.getMatrixFromQuaternion(com_orn)).reshape(3, 3)
        ankle_b_w = com_pos + R @ ankle_b_in_lower
        # Direction from ankle_b to pole_b
        d = pole_b_pos - ankle_b_w
        d_norm = d / np.linalg.norm(d)
        # Quaternion: rotate rb_dir0 → d_norm
        dot = float(np.clip(rb_dir0 @ d_norm, -1, 1))
        if dot > 0.9999:
            quat = [0, 0, 0, 1]
        else:
            axis = np.cross(rb_dir0, d_norm)
            axis = axis / np.linalg.norm(axis)
            ha = np.arccos(dot) / 2
            quat = [*(axis * np.sin(ha)), np.cos(ha)]
        p.resetBasePositionAndOrientation(rope_b_id,
                                          ankle_b_w.tolist(), quat)

    # --- Dynamics ---
    # linearDamping/angularDamping=0: remove PyBullet's default drag (0.04)
    # which does not exist in V10's equations of motion.
    p.changeDynamics(robot_id, ANKLE, mass=M1,
                     localInertiaDiagonal=box_inertia(M1, LOWER_W, BD, L1),
                     jointDamping=B_ALPHA,
    ...
    # Disable all self-collisions and ground collisions
    for i in range(-1, 3):
        for j in range(-1, 3):
            if i != j:
                p.setCollisionFilterPair(robot_id, robot_id, i, j, 0)
        p.setCollisionFilterPair(robot_id, 0, i, -1, 0)

    # --- State extraction ---
    def get_state():
        ra = p.getJointState(robot_id, ROPE_A)
        phi, phi_dot = ra[0], ra[1]
        ...
        up = p.getLinkState(robot_id, HIP, computeForwardKinematics=True,
                            computeLinkVelocity=True)
        lo_e = p.getEulerFromQuaternion(lo[5])
        up_e = p.getEulerFromQuaternion(up[5])
    ...
    def reset_pose():
        ...
        # Reset rope_b position to ankle_b
        p.resetBasePositionAndOrientation(
            rope_b_id, ankle_b_init.tolist(), [0,0,0,1])
        p.resetBaseVelocity(rope_b_id, [0,0,0], [0,0,0])
    ...
    try:
        while p.isConnected():
            ...
            if not paused:
                ...
                # Update visuals once per frame (not every substep)
                update_rope_b_visual()
                ...
                if step_n % 1000 == 0:
                    t = step_n * DT
                    ankle_q = p.getJointState(robot_id, ANKLE)[0]
                    print(f"  t={t:.1f}s  phi={np.degrees(x[0]):+.1f} "
                          f"a={np.degrees(x[1]):+.1f} "
                          f"th={np.degrees(x[2]):+.1f}  tau={tau:+.0f}"
                          f"  ankle_j={np.degrees(ankle_q):+.2f}")
