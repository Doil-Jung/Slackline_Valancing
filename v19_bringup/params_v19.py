...
CONTROL = dict(
    ...
    # FWE 프로파일 시간 (반복FEW 노트: 사이클 전체 < 배가시간대)
    T_FOLD = 0.08, T_WAIT = 0.06, T_EXTEND = 0.08, T_REST = 0.05,   # [s] ★소형은 배가~110ms: 사이클 전체를 그 부근으로
    RHO = 0.9,          # deadbeat 대비 게인 비 (최적 0.8~1.0, 과대접기 금지)
    A_TRIGGER_DEG = 0.4, # |A|가 이 값(β 환산 deg) 넘으면 접기 시작
    DELTA_MAX_DEG = 55.0,
)
