...
SL.Renderer3D = class {
    ...
    _buildScene() {
        ...
        // === 베타 라인 (발 → CoM) ===
        const betaLineGeo = new THREE.BufferGeometry();
        const betaLinePositions = new Float32Array([0, 0, 0, 0, 0, 0]);
        betaLineGeo.setAttribute('position', new THREE.BufferAttribute(betaLinePositions, 3));
        const betaLineMat = new THREE.LineBasicMaterial({
            color: 0xff006e,
            linewidth: 2,
            depthTest: false,
        });
        this.betaLine = new THREE.Line(betaLineGeo, betaLineMat);
        this.betaLine.renderOrder = 998;
        this.scene.add(this.betaLine);
        ...
        const zAxisPositions = new Float32Array([0, 0, 0, 0, 0, 4]);
        ...
        this.zAxisLine = new THREE.Line(zAxisGeo, zAxisMat);
        this.zAxisLine.computeLineDistances();
    ...
    _buildPoles() {
        ...
        const visible = p.showPoles !== false;
    ...
    _buildRopeTubes(ropePoints) {
        ...
        const tubeGeoA = new THREE.TubeGeometry(curveA, 1, 0.005, 6, false);
        ...
        const tubeGeoBottom = new THREE.TubeGeometry(curveBottom, 1, 0.005, 6, false);
        ...
        const tubeGeoB = new THREE.TubeGeometry(curveB, 1, 0.005, 6, false);
    ...
    update(state, model) {
        ...
        // === 베타 라인 (발 → CoM) ===
        const betaPositions = this.betaLine.geometry.attributes.position.array;
        betaPositions[0] = foot.x; betaPositions[1] = 0; betaPositions[2] = foot.z;
        betaPositions[3] = com.x;  betaPositions[4] = 0; betaPositions[5] = com.z;
        this.betaLine.geometry.attributes.position.needsUpdate = true;

        // === V자 줄 (숨김) ===
        this._updateRope(foot.x, foot.z);
        this.ropeLine.visible = false;
        if (this.ropeTubeA) this.ropeTubeA.visible = false;
        if (this.ropeTubeB) this.ropeTubeB.visible = false;
        if (this.ropeTubeBottom) this.ropeTubeBottom.visible = false;
        ...
        const showPoles = p.showPoles !== false;
        this.poleA.visible = showPoles;
        this.poleB.visible = showPoles;
        this.poleTopA.visible = showPoles;
        this.poleTopB.visible = showPoles;
    ...
    /** XZ 평면을 2D처럼 바라보는 사이드 뷰 */
    setSideView() {
        const p = this.params;
        const zCenter = this.Z_OFFSET + (p.L1 + p.L2) / 2;
        this.camera.position.set(0, -6, zCenter);
        this.controls.target.set(0, 0, zCenter);
        this.controls.update();
    }

    /** 기본 3D 뷰로 복원 */
    set3DView() {
        const zCenter = this.Z_OFFSET + 1.2;
        this.camera.position.set(4, 3, 3.5);
        this.controls.target.set(0, 0, zCenter);
        this.controls.update();
    }
