/**
 * Slackline Balance Simulator V16 — Main Entry Point
 * 
...
(function () {
    ...
    function init() {
        ...
        console.log('Slackline Balance Simulator V16 (True Equation 4 FWE) initialized');
    }
    ...
    function updateInfoOverlay(state, model, controller) {
        ...
        const pp2 = m2 * ell2;
        const x_com = p.R * Math.sin(state.phi)
                     + (pp1 * Math.sin(state.alpha) + pp2 * Math.sin(state.theta)) / Mt;
    ...
    function setupFWESliders() {
        const sliders = [
            { id: 'slider-fwe-k1',         valId: 'val-fwe-k1',         prop: 'k1',             scale: 1,             precision: 2 },
        ...
        sliders.forEach(s => {
            ...
            slider.addEventListener('input', () => {
                ...
                console.log(`[V16 UI] ${s.prop} = ${controller[s.prop]}`);
            });
        });
    }
    ...
    function computeBetaDelta(state, model) {
        ...
        const p2 = m2 * ell2;
        const R = p.R;
        const x_com = R * Math.sin(state.phi)
                     + (p1 * Math.sin(state.alpha) + p2 * Math.sin(state.theta)) / Mt;
        const beta = x_com / h_com;
        const delta = state.theta - state.alpha;
        return { beta, delta };
    }
