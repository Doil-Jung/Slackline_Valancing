/**
 * Slackline Balance Simulator — Canvas Renderer (2-Body Model)
 * 
 * 하체+상체 2개 직사각형, 힙 관절, 원호 트랙, CoM, 실시간 그래프
 */
var SL = SL || {};
...
SL.Renderer = class {
    ...
    drawSim(state, model) {
        ...
        // === 하체 (발→힙) ===
        this.drawSegment(ctx, foot, hip, p.lowerWidth, -state.phi,
            'rgba(6,214,160,0.55)', this.colors.lowerOutline);

        // === 상체 (힙→머리) ===
        this.drawSegment(ctx, hip, head, p.upperWidth, state.theta,
    ...
    drawSegment(ctx, from, to, width, angle, fillColor, strokeColor) {
        ...
        // 캔버스 회전 각도 계산
        // dy = toC.cy - fromC.cy (캔버스는 아래가 +y)
        // dx = toC.cx - fromC.cx
        // 직사각형을 세로(-len/2 부터 len/2)로 그리므로, 기본 방향은 위쪽(-y)임.
        // 따라서 Math.PI / 2 를 더해주어 방향을 맞춤.
    ...
    drawInfo(ctx, state, model) {
        ...
        const hipDeg = ((state.theta + state.phi) * 180 / Math.PI).toFixed(1);
        ...
        ctx.fillRect(x - 5, y - 16, 170, lines.length * 20 + 8);
