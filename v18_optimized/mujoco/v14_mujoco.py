"""
V18 MuJoCo Slackline Balancing — Optimized Small Robot Shape
============================================================
...
# 최적화 형상: H=80cm, L1:L2=30:70, 총 3kg, R=0.5m
# (형상최적화_상하체비율_20260615.md §3)
M1 = _env('V14_M1', 0.80); M2 = _env('V14_M2', 2.20)  # 상체+머리 합산
L1 = _env('V14_L1', 0.24); L2 = _env('V14_L2', 0.56)
BD = _env('V14_BD', 0.10)
LOWER_W = L2 / 4; UPPER_W = L2 / 4   # body thickness ∝ L2
B_THETA = _env('V14_BTHT', 0.01); TAU_MAX = _env('V14_TAUMAX', 99999); GRAV = 9.81

# Head 비활성화 (해석 모형과 일치)
M_HEAD = _env('V14_MHEAD', 0.0)
HEAD_R = L2 / 4

L_POLE = _env('V14_LPOLE', 0.80); POLE_H = _env('V14_POLEH', 1.00)
R_TARGET = _env('V14_RTGT', 0.50)
...
R_COST = _env('V14_RCOST', 5.0)    # 소형: R_COST=0.001이면 |eig|=8 발산, 5.0이면 |eig|≈1.001 안정
ROPE_MASS = 0.01
...
{f'          <geom name="head" type="sphere" size="{HEAD_R}" pos="0 0 {L2 + HEAD_R}" mass="{M_HEAD}" rgba="1.0 0.85 0.7 0.9"/>' if M_HEAD > 0.01 else '          <!-- head disabled -->'}
        </body>
      </body>
    </body>
...
def compute_lqr():
    ...
    K[0,0] *= -1; K[0,3] *= -1  # phi convention (v14 원본)
...
def main():
    ...
    print("="*60)
    print("  V18 MuJoCo Slackline — Optimized Small Robot")
    ...
    # Simulation state
    paused = True
    ctrl_on = True   # 소형 로봇: 기본 제어기 ON
    step_n = 0

    print("[Sim] Space=Pause  R=Reset  C=Toggle_Ctrl  Q=Quit")
    print(f"[Sim] Controller ON (press C to toggle)")
