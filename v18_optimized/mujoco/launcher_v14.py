"""
V18 MuJoCo Launcher — Parameter GUI (Optimized Small Robot)
...
class Launcher:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("V18 MuJoCo Slackline Launcher — Optimized Shape")
    ...
    def _build_ui(self):
        ...
        self._add_slider(root, r, "Lower mass M1 (kg):",   0.05, 5.0, 0.80,  'm1', ".2f"); r+=1
        self._add_slider(root, r, "Upper mass M2 (kg):",   0.05, 5.0, 2.20,  'm2', ".2f"); r+=1
        self._add_slider(root, r, "Head mass (kg):",        0,  2.0, 0.00,  'mh', ".2f"); r+=1
        self._add_slider(root, r, "Lower length L1 (m):",  0.05, 0.50, 0.24, 'l1', ".3f"); r+=1
        self._add_slider(root, r, "Upper length L2 (m):",  0.10, 0.80, 0.56, 'l2', ".3f"); r+=1
        self._add_slider(root, r, "Body width BD (m):",    0.03, 0.30, 0.10, 'bd', ".3f"); r+=1

        # === Structure ===
        ttk.Label(root, text="── Structure ──", font=('', 10, 'bold')).grid(
            row=r, column=0, columnspan=3, pady=(10,4)); r+=1

        self._add_slider(root, r, "Pole spacing L_pole (m):", 0.1, 3.0, 0.80, 'lpole', ".3f"); r+=1
        self._add_slider(root, r, "Pole height (m):",         0.3, 3.0, 1.00, 'poleh', ".3f"); r+=1
        self._add_slider(root, r, "Sag / R_target (m):",      0.05, 2.0, 0.50, 'rtgt', ".3f"); r+=1
        ...
        self._add_slider(root, r, "Hip damping B_th:",      0, 5.0,  0.01, 'btht', ".3f"); r+=1
        self._add_slider(root, r, "Max torque (Nm):",      0.1, 100000, 99999, 'taumax', ".0f"); r+=1
        self._add_slider(root, r, "LQR R_cost:",        0.0001, 50.0, 5.0, 'rcost', ".3f"); r+=1
    ...
    def _reset_defaults(self):
        defaults = {
            'm1': 0.80, 'm2': 2.20, 'mh': 0.00,
            'l1': 0.24, 'l2': 0.56, 'bd': 0.10,
            'lpole': 0.80, 'poleh': 1.00, 'rtgt': 0.50,
            'theta0': 8.6, 'alpha0': 0.0,
            'btht': 0.01, 'taumax': 99999, 'rcost': 5.0,
            'q0': 1, 'q1': 50, 'q2': 50,
            'q3': 0.1, 'q4': 5, 'q5': 5,
        }
