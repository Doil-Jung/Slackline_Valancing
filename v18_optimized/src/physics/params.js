/**
 * Slackline Balance Simulator V18 — Physical Parameters (Optimized Small Robot)
 * 
 * 형상최적화_상하체비율_20260615.md §3 추천 형상 적용
 * H=80cm, L1:L2=30:70, 총 3kg, R=0.5m
 * 
...
SL.Params = {
    // === 하체 (다리) — 발끝 가볍게, 질량은 힙 근처 ===
    m1: 0.80,       // 하체 질량 (kg) — 발끝 비움, 힙 근처 배치
    L1: 0.24,       // 하체 길이 — 발에서 힙 (m) — H×0.30

    // === 상체 (몸통) ===
    m2: 1.70,       // 상체 질량 (kg) — 구조체
    L2: 0.56,       // 상체 길이 — 힙에서 머리 (m) — H×0.70

    // === 줄 ===
    R: 0.50,        // 원호 반경 = 처짐 깊이 (m)
    g: 9.81,        // 중력 가속도 (m/s²)

    // === 렌더링 치수 (소형) ===
    lowerWidth: 0.04,  // 하체 폭 (m)
    upperWidth: 0.06,  // 상체 폭 (m)

    // === 감쇠 (소형 스케일) ===
    b_phi: 0.2,     // 원호 운동 감쇠 (N·m·s/rad)
    b_alpha: 0.1,   // 하체 회전 감쇠 (N·m·s/rad)
    b_theta: 0.05,  // 상체 회전 감쇠 (N·m·s/rad)
    ...
    ranges: {
        R:       { min: 0.1, max: 2.0, step: 0.05, label: '원호 반경 R (m)' },
        m1:      { min: 0.1, max: 5.0, step: 0.1,  label: '하체 질량 m₁ (kg)' },
        m2:      { min: 0.1, max: 5.0, step: 0.1,  label: '상체 질량 m₂ (kg)' },
        L1:      { min: 0.10, max: 0.50, step: 0.01, label: '하체 길이 L₁ (m)' },
        L2:      { min: 0.20, max: 0.80, step: 0.01, label: '상체 길이 L₂ (m)' },
        b_phi:   { min: 0,   max: 2.0, step: 0.01,  label: '원호 감쇠 b_φ' },
        b_alpha: { min: 0,   max: 2.0, step: 0.01,  label: '하체 감쇠 b_α' },
        b_theta: { min: 0,   max: 1.0, step: 0.01,  label: '상체 감쇠 b_θ' },
