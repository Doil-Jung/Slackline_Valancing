...
(function () {
    ...
    function init() {
        ...
        // 하드코딩 게인이 대형 로봇 기준이므로 현재 파라미터로 즉시 재계산
    ...
    function setupSliders() {
        ...
        Object.keys(ranges).forEach(key => {
            ...
            slider.addEventListener('input', () => {
                ...
                renderer3d.rebuild();
                // 공칭값 직접 갱신 후 편차 재적용
                // (captureNominal()은 이전 공칭값을 먼저 복원하므로 슬라이더 변경이 무시됨)
                paramManager._nominalSnapshot[key] = val;
                applyMismatch();
                // 물리 파라미터 변경 시 LQR 게인 + 옵저버 재초기화
                if (['m1', 'm2', 'L1', 'L2', 'R'].includes(key)) {
                    const rScale = controller.rScale || 1.0;
                    controller.recomputeBaseGain(paramManager.nominalParams, rScale);
                    if (observer) {
                        const nominalModel = new SL.Model(paramManager.nominalParams);
                        observer.init(paramManager.nominalParams, controller, nominalModel);
                        if (model) observer.resetState(model.state);
                    }
                }
            });
        });
