...
(function () {
    ...
    function init() {
        ...
        // 현재 파라미터로 DARE를 풀어 LQR 게인 재계산 (하드코딩 값 대체)
        controller.recomputeBaseGain(paramManager.nominalParams, 1.0);
        console.log('=== V10 INIT: LQR Gains ===');
        console.log('Params:', JSON.stringify({m1: SL.Params.m1, m2: SL.Params.m2, L1: SL.Params.L1, L2: SL.Params.L2, R: SL.Params.R}));
        console.log('K_base:', controller.K_base.map(v => v.toFixed(4)));
    ...
    function updateInfoOverlay(state, model, controller) {
        ...
        let html = `
            ...
            <div>힙각 = ${hipDeg}°</div>
            <div>τ = ${model.tau.toFixed(3)} N·m</div>
            <div style="font-size:0.75em; opacity:0.4;">K=[${controller.K_base.slice(0,3).map(v=>v.toFixed(1)).join(',')}...]</div>
    ...
    function setupSliders() {
        ...
        Object.keys(ranges).forEach(key => {
            ...
            slider.addEventListener('input', () => {
                ...
                paramManager.captureNominal();
                applyMismatch();
                // 물리 파라미터 변경 시 LQR 게인 재계산 (DARE)
                if (['m1','m2','L1','L2','R','b_phi','b_alpha','b_theta'].includes(key)) {
        ...
        // === R 자동 계산: R = sqrt((L0/2)² - (L_pole/2)²) ===
        function recomputeR() {
            const l = SL.Params.L0 / 2;
            const d = SL.Params.L_pole / 2;
            if (l <= d) {
                // 줄이 기둥 간격보다 짧으면 최소값 사용
                SL.Params.R = 0.01;
            } else {
                SL.Params.R = Math.sqrt(l * l - d * d);
            }
            // R 표시 업데이트
            const rDisplay = document.getElementById('val-R');
    ...
    function loop() {
        ...
        // 발산 감지: 시뮬레이션 자동 정지
        if (model._diverged) {
            console.error('🛑 Simulation stopped: state diverged at t=' + model.state.time.toFixed(3));
            stop();
            const btnStartStop = document.getElementById('btn-start-stop');
            if (btnStartStop) {
                btnStartStop.textContent = '⚠ 발산! ▶ 재시작';
                btnStartStop.classList.remove('active');
            }
            model._diverged = false; // 리셋 후 재시작 가능
            updateInfoOverlay(model.state, model, controller);
            return;
        }
