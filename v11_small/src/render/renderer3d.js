...
SL.Renderer3D = class {
    constructor(container, params) {
        ...
        this.params = params;
        this.Z_OFFSET = 0.3; // 전체 높이 오프셋 (바닥에서 띄움)

        // === Three.js 기본 설정 ===
        this.scene = new THREE.Scene();
        this.scene.background = new THREE.Color(0x0a0a1a);
        this.scene.fog = new THREE.FogExp2(0x0a0a1a, 0.15);

        // 카메라
        const aspect = container.clientWidth / container.clientHeight;
        this.camera = new THREE.PerspectiveCamera(50, aspect, 0.01, 50);
        this.camera.position.set(1.2, 0.8, 1.0);
        ...
        const robotMidZ = this.Z_OFFSET + (this.params.L1 + this.params.L2) / 2;
    ...
    _buildScene() {
        ...
        // === CoM 마커 ===
        const comSize = Math.max(0.008, (this.params.L1 + this.params.L2) * 0.03);
        const comGeo = new THREE.OctahedronGeometry(comSize, 0);
        ...
        const maxRefH = this.params.L1 + this.params.L2 + 0.1;
        const refLinePositions = new Float32Array([0, 0, 0, 0, 0, maxRefH]);
    ...
    _buildPoles() {
        ...
        // 기둥 상단 마커
        const topRadius = Math.max(0.005, poleRadius * 1.5);
        const topGeo = new THREE.SphereGeometry(topRadius, 12, 12);
    ...
    _buildHipJoint() {
        ...
        const hipRadius = Math.max(0.003, bodyDepth * 0.06);
        const hipGeo = new THREE.CylinderGeometry(hipRadius, hipRadius, bodyDepth * 1.5, 12);
