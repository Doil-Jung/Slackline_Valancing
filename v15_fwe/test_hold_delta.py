"""
Check: when delta is held constant at +20deg,
how do phi and beta evolve? Is beta bounded?
"""
...
# Hold delta constant, starting from beta=5d
...
for dd in [0, 10, 20, 30, 40]:
    ...
    ts=np.array(ts)*1000
    axes[0].plot(ts, np.degrees(phis), lw=1.5, label=f'd={dd}d')
    axes[1].plot(ts, np.degrees(betas), lw=1.5, label=f'd={dd}d')

    # Print phi half-period (time for phi to return to 0)
    phis_arr = np.array(phis)
    crosses = np.where(np.diff(np.sign(phis_arr)))[0]
    if len(crosses) > 0:
        t_cross = ts[crosses[0]]
        print(f"  delta={dd}d: phi crosses 0 at {t_cross:.0f}ms, beta@cross={np.degrees(betas[crosses[0]]):.1f}d")
    else:
        print(f"  delta={dd}d: phi never crosses 0 in 1s")

axes[0].set_ylabel('phi [d]'); axes[0].legend(fontsize=9)
axes[0].axhline(0, color='red', ls='--')
axes[1].set_ylabel('beta [d]'); axes[1].legend(fontsize=9)
axes[1].axhline(0, color='black', lw=0.5)
axes[1].set_xlabel('ms')

# Also test: FOLD then immediately EXTEND (T_w1=0)
print("\n--- Fold-then-immediately-extend test ---")
for dd in [10, 20, 30]:
    data = mujoco.MjData(model)
    data.qpos[model.jnt_qposadr[model.joint('ankle').id]] = np.radians(5)
    mujoco.mj_forward(model, data)
    ...
    N=int(1.0/DT)
    ts=[];phis=[];betas=[];dacts=[]
    t_fold=0.05  # fold for 50ms then extend
    for i in range(N):
        phi=data.joint('phi_Y').qpos[0]
        beta=compute_beta(model,data)
        t = data.time
        if t < t_fold:
            ...
            data.ctrl[0] = 0
        ts.append(t);phis.append(phi);betas.append(beta);dacts.append(data.joint('hip').qpos[0])
        mujoco.mj_step(model,data)

    ts=np.array(ts)*1000
    axes[2].plot(ts, np.degrees(betas), lw=1.5, label=f'd={dd}d 50ms')
    
    betas_arr = np.degrees(np.array(betas))
    phis_arr = np.array(phis)
    # Find phi=0 after extend
    crosses = np.where(np.diff(np.sign(phis_arr)))[0]
    for c in crosses:
        if ts[c] > 50:  # after extend
            print(f"  d={dd}d: phi=0 at {ts[c]:.0f}ms, beta={betas_arr[c]:.1f}d")
            break

axes[2].set_ylabel('beta [d] (fold+extend)'); axes[2].legend(fontsize=9)
axes[2].axhline(0, color='black', lw=0.5)
axes[2].set_xlabel('ms')
axes[0].set_title('Hold delta constant from beta0=5d | Bottom: fold 50ms then extend')
plt.tight_layout()
out=os.path.join(os.path.dirname(os.path.abspath(__file__)),'v15_hold_delta.png')
