"""
V15 MuJoCo Slackline Balancing — FWE 4-Phase Control
=====================================================
Based on v14 model (closed kinematic loop).
Replaces LQR torque control with FWE position-servo control.
"""
import os, sys, time
import numpy as np
import mujoco
...
# ============================================================
#  Robot Parameters (small robot, no head)
# ============================================================
M1 = 0.1          # lower body mass [kg]
M2 = 0.25         # upper body mass [kg]
L1 = 0.2          # lower body length [m]
L2 = 0.2          # upper body length [m]
BD = 0.10         # body depth (between ankles) [m]
GRAV = 9.81
DT = 0.001        # simulation timestep

# Rope / pole geometry
L_POLE = 0.8; POLE_H = 1.0
R_TARGET = 0.3    # rope length [m]

# Servo PD gains (stiff position tracking)
KP_SERVO = 200.0
KD_SERVO = 4.0

# Joint damping
B_THETA = 0.01    # hip joint damping

# Initial condition: beta0 ≈ 5°
ALPHA0 = np.radians(5.0)   # lower body tilt
THETA0 = np.radians(5.0)   # upper body tilt (same = uniform tilt)

# FWE parameters (from robot convergence analysis)
FWE_EPS = 12.0
FWE_TF  = 0.020   # 20ms fold/extend time
FWE_TW1 = 0.260   # 260ms wait time
...
<mujoco model="slackline_v15_fwe">
  ...
  <worldbody>
    <!-- Ground -->
    <geom type="plane" size="3 3 0.01" rgba="0.3 0.3 0.35 1"
          contype="1" conaffinity="1"/>
    <light pos="2 0 4" dir="-0.3 0 -1" diffuse="0.8 0.8 0.8"/>
    <light pos="-2 0 4" dir="0.3 0 -1" diffuse="0.4 0.4 0.4"/>
...
def main():
    print("=" * 60)
    print("  V15 MuJoCo — FWE 4-Phase Control")
    print(f"  m1={M1} m2={M2}  L1={L1} L2={L2}  R={R_TARGET}")
    print(f"  h_com={h_com:.4f}m  c_foot={c_foot:.4f}")
    print(f"  FWE: eps={FWE_EPS} Tf={FWE_TF*1000:.0f}ms "
          f"Tw1={FWE_TW1*1000:.0f}ms thresh={np.degrees(FWE_BETA_THRESH):.1f}d")
    print(f"  Servo: kp={KP_SERVO} kd={KD_SERVO}")
    ...
    data.qpos[hip_qadr] = THETA0 - ALPHA0  # hip = relative
    ...
    # CSV log
    log_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'v15_log.csv')
