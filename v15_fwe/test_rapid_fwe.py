"""
Rapid FWE: fold for T_fold ms, extend immediately (T_w1=0),
wait for phi=0, repeat. Check if beta converges.
"""
...
def run_rapid_fwe(eps, T_w1, beta0_deg=5, sim_time=3.0, max_cycles=20):
    """
    Rapid FWE: FOLD→WAIT1(T_w1)→EXTEND→WAIT2(phi=0)→repeat
    """
    model = mujoco.MjModel.from_xml_string(xml)
    data = mujoco.MjData(model)
    data.qpos[model.jnt_qposadr[model.joint('ankle').id]]=np.radians(beta0_deg)
    mujoco.mj_forward(model,data)
    max_d=np.radians(45); arrive_th=np.radians(2)
    ...
    for i in range(N):
        ...
        if state==IDLE:
            ...
            if abs(hip_act-target)<arrive_th or t_ph>0.15:
                state=WAIT1;t_ph=0
        elif state==WAIT1:
            target=sign*d_fold
            if t_ph>=T_w1:
...
configs = [
    (4, 0.00),   # eps=4,  T_w1=0ms (immediate extend)
    (4, 0.02),   # eps=4,  T_w1=20ms
    (4, 0.05),   # eps=4,  T_w1=50ms
    (2, 0.00),   # eps=2,  T_w1=0ms
    (2, 0.02),   # eps=2,  T_w1=20ms
    (8, 0.02),   # eps=8,  T_w1=20ms
]
ncol = len(configs)
fig, axes = plt.subplots(3, ncol, figsize=(5*ncol, 10), sharex='col')

for col, (eps, tw1) in enumerate(configs):
    ts,phis,betas,dcmd,dact,sts,bai = run_rapid_fwe(eps,tw1,5,3.0)
    ...
    axes[0,col].set_title(f'e={eps} Tw1={tw1*1000:.0f}ms',fontsize=10)
    ...
    print(f"eps={eps} Tw1={tw1*1000:.0f}ms: beta@idle = {bai}")

plt.suptitle('Rapid FWE (short Tw1)',fontsize=13)
plt.tight_layout()
out=os.path.join(os.path.dirname(os.path.abspath(__file__)),'v15_rapid_fwe.png')
