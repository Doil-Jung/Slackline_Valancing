"""Servo-feasible search with wider T_f range (300ms-1000ms)"""
import numpy as np
...
eps_vals = np.arange(2.0, 8.5, 0.5)
tf_vals = [0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 1.00]
tw1_vals = np.arange(0.05, 2.05, 0.05)

print(f"Grid: {len(eps_vals)} eps x {len(tf_vals)} Tf x {len(tw1_vals)} Tw1")

all_conv = []
for eps in eps_vals:
    for T_f in tf_vals:
        d_fold = h_com*(1+eps)*beta0/c_foot
        peak_vel = np.degrees(2*d_fold/T_f)
    ...
    print(f"  eps={eps:.1f} done")

print(f"\n{'='*80}")
print(f"Found {len(all_conv)} servo-feasible convergent cases!")
print(f"{'='*80}")

if all_conv:
    all_conv.sort(key=lambda x: abs(x['r']))
    print(f"\n{'eps':>5} {'Tf':>5} {'Tw1':>6} | {'Tw2':>6} {'Ttot':>6} | "
          f"{'r':>8} | {'dfold':>5} {'vmax':>6} | note")
    print("-" * 80)
    for c in all_conv[:30]:
        ttot_s = c['T_tot']
        print(f"{c['eps']:5.1f} {c['T_f']*1000:4.0f}ms {c['T_w1']*1000:5.0f}ms | "
              f"{c['T_w2']*1000:5.0f}ms {ttot_s:.2f}s | "
              f"{c['r']:8.4f} | {c['d_fold_deg']:4.0f}d {c['peak_vel']:5.0f}d/s | "
              f"b={np.degrees(c['beta_f']):.2f}d")
