"""
Quadratic FWE:
  beta_eff = beta + beta_dot * T_fold
  delta = K1 * beta_eff * |beta_eff|   (sign preserved, magnitude ~ beta_eff^2)
  
  T_fold = 20ms, servo ramps linearly
  Sweep: K1 (fine), T_w1 (coarse)
"""
...
def run_fwe(K1, T_w1, beta0_deg=5, sim_time=8.0, max_cyc=20, record=False):
    ...
    beta_eff = beta  # first cycle beta_dot=0
    # QUADRATIC: delta = K1 * beta_eff * |beta_eff|
    delta_target = np.clip(K1 * beta_eff * abs(beta_eff), -md, md)
    delta_start = 0.0
...
# === Sweep ===
# K1 units are different now: delta ~ K1 * beta^2
# For beta=5deg=0.087rad: K1*0.087^2 = K1*0.0076
# To get delta~20deg=0.35rad: K1 = 0.35/0.0076 = 46
# So K1 should be much larger than before
K1_vals = np.arange(30, 81, 2)   # 30~80 step 2 (26 values)
Tw1_vals = np.arange(0.040, 0.101, 0.005)  # 40~100ms step 5ms (13 values)

total = len(K1_vals) * len(Tw1_vals)
t0 = time.time()
bai_test = run_fwe(50, 0.06)
dt_case = time.time() - t0

print(f"QUADRATIC FWE: delta = K1 * beta_eff * |beta_eff|")
print(f"T_FOLD = {T_FOLD*1000:.0f}ms")
print(f"K1: {K1_vals[0]} ~ {K1_vals[-1]} step 5 ({len(K1_vals)})")
print(f"Tw1: {[f'{t*1000:.0f}ms' for t in Tw1_vals]} ({len(Tw1_vals)})")
print(f"Total: {total}, est {total*dt_case:.0f}s")
print(f"Test K1=50 Tw1=60ms: {[f'{b:+.1f}' for b in bai_test]}")
...
for K1 in K1_vals:
    for tw in Tw1_vals:
        ...
        if bai:
            mx = max(abs(b) for b in bai[:10])
            results.append((mx, len(bai), int(K1), tw, bai))
...
for rank, (score, nc, k1, tw, bai) in enumerate(results[:30]):
    ...
    if nc >= 4:
        ...
        if all(a < 5 for a in ab[:6]): conv = "***"
        elif all(a < 10 for a in ab[:5]): conv = "**"
        elif all(a < 20 for a in ab[:4]): conv = "*"
        elif all(a < 30 for a in ab[:4]): conv = "~"
    print(f"{rank+1:3d} {k1:5d} {tw*1000:5.0f}ms {nc:3d} {score:6.1f}  [{bai_s}]  {conv}")
...
for lim in [5, 10, 20, 30]:
    ...
    for s, nc, k1, tw, bai in matches[:10]:
        bai_s = ", ".join([f"{b:+.1f}" for b in bai[:12]])
        print(f"  K1={k1} Tw1={tw*1000:.0f}ms: [{bai_s}]")
...
if results:
    ...
    axes[0].set_title(f"QUADRATIC: K1={bk1} Tw1={btw*1000:.0f}ms  delta=K1*b_eff*|b_eff|")
    ...
    plt.tight_layout(); plt.savefig('v15_quadratic.png', dpi=150)
    print("Saved v15_quadratic.png")
