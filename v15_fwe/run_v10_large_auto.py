# -*- coding: utf-8 -*-
"""
V15 Headless Test — Run Large Robot FWE and plot convergence
"""
import os, sys
import numpy as np
import mujoco
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from fwe_controller import FWEController, Phase

# ============================================================
#  Robot Parameters (large-scale robot)
# ============================================================
M1 = 30.0         # lower body mass [kg]
M2 = 45.0         # upper body mass [kg]
L1 = 0.85         # lower body length [m]
L2 = 0.85         # upper body length [m]
BD = 0.40         # body depth (between ankles) [m]
GRAV = 9.81
DT = 0.001        # simulation timestep

# Rope / pole geometry
L_POLE = 3.0; POLE_H = 3.0
R_TARGET = 1.5    # rope length [m]

# Servo PD gains (stiff position tracking for high mass)
KP_SERVO = 150000.0
KD_SERVO = 3000.0
TAU_MAX = 100000.0

# Joint damping
B_THETA = 0.5     # hip joint damping

# Initial condition: beta0 ≈ 5°
ALPHA0 = np.radians(5.0)   # lower body tilt
THETA0 = np.radians(5.0)   # upper body tilt (same = uniform tilt)

# FWE parameters (from large model sweep)
FWE_EPS = 3.0
FWE_TF  = 0.200   # 200ms fold/extend time
FWE_TW1 = 0.300   # 300ms wait time
FWE_BETA_THRESH = np.radians(1.0)  # trigger threshold

# Derived
LC1 = L1/2; LC2 = L2/2
Mt = M1 + M2
I1 = M1*L1**2/12; I2 = M2*L2**2/12
p1 = M1*LC1 + M2*L1;  p2 = M2*LC2;  ps = p1 + p2
h_com = (M1*LC1 + M2*(L1+LC2)) / Mt
c_foot = p2 / ps

HALF_POLE = L_POLE / 2.0
ROPE_HORIZ = HALF_POLE - BD / 2.0
ROPE_VERT  = R_TARGET
FOOT_Z = POLE_H - ROPE_VERT
ROPE_MASS = 0.5  # Heavy rope for large-scale stability

# Geometry sizes
LOWER_W = L2/4; UPPER_W = L2/4
POLE_RADIUS = 0.05
POLE_TOP_R  = 0.08
ROPE_RADIUS = 0.015
HIP_R       = 0.08
HIP_H       = 0.05


def generate_mjcf():
    rh = ROPE_HORIZ; rv = ROPE_VERT; hp = HALF_POLE
    lw2 = LOWER_W/2; uw2 = UPPER_W/2; bd2 = BD/2

    xml = f"""<?xml version="1.0"?>
<mujoco model="slackline_v15_large_fwe">
  <option gravity="0 0 -{GRAV}" timestep="{DT}" iterations="200" tolerance="1e-10">
    <flag contact="disable"/>
  </option>

  <default>
    <geom contype="0" conaffinity="0"/>
    <joint damping="0" armature="0.01"/>
  </default>

  <worldbody>
    <geom type="plane" size="6 6 0.01" rgba="0.3 0.3 0.35 1" contype="1" conaffinity="1"/>
    <light pos="4 0 8" dir="-0.3 0 -1" diffuse="0.8 0.8 0.8"/>
    <light pos="-4 0 8" dir="0.3 0 -1" diffuse="0.4 0.4 0.4"/>

    <!-- Pole A -->
    <geom name="pole_a_cyl" type="cylinder" pos="0 {hp} {POLE_H/2}" size="{POLE_RADIUS} {POLE_H/2}" rgba="0.5 0.5 0.5 1"/>
    <geom name="pole_a_top" type="sphere" pos="0 {hp} {POLE_H}" size="{POLE_TOP_R}" rgba="0.9 0.9 0.9 1"/>

    <!-- Pole B -->
    <geom name="pole_b_cyl" type="cylinder" pos="0 {-hp} {POLE_H/2}" size="{POLE_RADIUS} {POLE_H/2}" rgba="0.5 0.5 0.5 1"/>
    <geom name="pole_b_top" type="sphere" pos="0 {-hp} {POLE_H}" size="{POLE_TOP_R}" rgba="0.9 0.9 0.9 1"/>

    <!-- ============ ROPE A CHAIN ============ -->
    <body name="rope_a_mount" pos="0 {hp} {POLE_H}">
      <joint name="phi_Y" type="hinge" axis="0 1 0"/>
      <joint name="rope_a_X" type="hinge" axis="1 0 0"/>
      <geom name="rope_a" type="capsule" size="{ROPE_RADIUS}" fromto="0 0 0 0 {-rh} {-rv}" rgba="1 0.8 0 1" mass="{ROPE_MASS}"/>

      <!-- Lower body -->
      <body name="lower_body" pos="0 {-rh} {-rv}">
        <joint name="ankle" type="hinge" axis="0 1 0"/>
        <geom name="lower_geom" type="box" size="{lw2} {bd2} {L1/2}" pos="0 {-bd2} {L1/2}" mass="{M1}" rgba="0.02 0.84 0.63 0.8"/>
        <body name="ankle_b_target" pos="0 {-BD} 0"/>
        <geom name="hip_cyl" type="cylinder" size="{HIP_R} {HIP_H}" pos="0 {-bd2} {L1}" euler="90 0 0" rgba="1 0.75 0.04 1" mass="0.01"/>

        <!-- Upper body -->
        <body name="upper_body" pos="0 {-bd2} {L1}">
          <joint name="hip" type="hinge" axis="0 1 0" damping="{B_THETA}"/>
          <geom name="upper_geom" type="box" size="{uw2} {bd2} {L2/2}" pos="0 0 {L2/2}" mass="{M2}" rgba="0.51 0.22 0.93 0.8"/>
        </body>
      </body>
    </body>

    <!-- ============ ROPE B CHAIN ============ -->
    <body name="rope_b_mount" pos="0 {-hp} {POLE_H}">
      <joint name="rope_b_Y" type="hinge" axis="0 1 0"/>
      <joint name="rope_b_X" type="hinge" axis="1 0 0"/>
      <geom name="rope_b" type="capsule" size="{ROPE_RADIUS}" fromto="0 0 0 0 {rh} {-rv}" rgba="1 0.8 0 1" mass="{ROPE_MASS}"/>
      <body name="rope_b_end" pos="0 {rh} {-rv}"/>
    </body>
  </worldbody>

  <equality>
    <connect body1="rope_b_end" body2="ankle_b_target" anchor="0 0 0" solref="0.001 1" solimp="0.999 0.999 0.0001"/>
  </equality>

  <actuator>
    <general name="hip_servo" joint="hip" biastype="affine" gainprm="{KP_SERVO} 0 0" biasprm="0 {-KP_SERVO} {-KD_SERVO}" forcerange="{-TAU_MAX} {TAU_MAX}" forcelimited="true" ctrlrange="-3.14 3.14" ctrllimited="true"/>
  </actuator>
</mujoco>"""
    return xml


def get_state(model, data):
    phi = data.joint('phi_Y').qpos[0]
    lb_id = model.body('lower_body').id
    xmat = data.xmat[lb_id].reshape(3, 3)
    alpha = np.arctan2(-xmat[2, 0], xmat[2, 2])

    ub_id = model.body('upper_body').id
    xmat_u = data.xmat[ub_id].reshape(3, 3)
    theta = np.arctan2(-xmat_u[2, 0], xmat_u[2, 2])
    return phi, alpha, theta


def main():
    print("=" * 60)
    print("  V15 MuJoCo Headless - Large-scale Robot FWE Simulation")
    print(f"  m1={M1}kg  m2={M2}kg  Mt={Mt}kg  L1={L1}m  L2={L2}m  R={R_TARGET}m")
    print(f"  FWE parameters: eps={FWE_EPS} Tf={FWE_TF*1000:.0f}ms Tw1={FWE_TW1*1000:.0f}ms")
    print(f"  Servo parameters: kp={KP_SERVO} kd={KD_SERVO}")
    print("=" * 60)

    # Build model
    xml = generate_mjcf()
    model = mujoco.MjModel.from_xml_string(xml)
    data = mujoco.MjData(model)

    # Joint addresses
    hip_jid = model.joint('hip').id
    hip_qadr = model.jnt_qposadr[hip_jid]
    ankle_jid = model.joint('ankle').id
    ankle_qadr = model.jnt_qposadr[ankle_jid]

    # Set initial pose
    data.qpos[ankle_qadr] = ALPHA0
    data.qpos[hip_qadr] = THETA0 - ALPHA0
    mujoco.mj_forward(model, data)

    # Create FWE controller
    fwe = FWEController(
        eps=FWE_EPS, T_f=FWE_TF, T_w1=FWE_TW1,
        h_com=h_com, c_foot=c_foot,
        p1=p1, p2=p2, Mt=Mt,
        beta_threshold=FWE_BETA_THRESH,
        max_delta=np.radians(90.0)
    )

    SIM_TIME = 6.0
    N = int(SIM_TIME / DT)

    # Logging lists
    t_arr = np.zeros(N)
    phi_arr = np.zeros(N)
    beta_arr = np.zeros(N)
    dcmd_arr = np.zeros(N)
    dact_arr = np.zeros(N)
    phase_arr = np.zeros(N, dtype=int)

    print("Running simulation...")
    for i in range(N):
        phi, alpha, theta = get_state(model, data)
        beta = (p1*alpha + p2*theta) / (Mt*h_com)
        delta_actual = data.joint('hip').qpos[0]

        # FWE step (always active)
        delta_cmd = fwe.step(DT, phi, alpha, theta)
        data.ctrl[0] = delta_cmd

        t_arr[i] = data.time
        phi_arr[i] = phi
        beta_arr[i] = beta
        dcmd_arr[i] = delta_cmd
        dact_arr[i] = delta_actual
        phase_arr[i] = list(Phase).index(fwe.phase)

        if i % 50 == 0:
            print(f"  t={data.time*1000:.0f}ms | phi={np.degrees(phi):+6.2f} | beta={np.degrees(beta):+6.2f} | cmd={np.degrees(delta_cmd):+6.2f} | act={np.degrees(delta_actual):+6.2f} | phase={fwe.phase_name}")

        mujoco.mj_step(model, data)

        if np.any(np.isnan(data.qpos)):
            print(f"[!] NaN encountered at t={data.time:.3f}s")
            # Truncate arrays
            t_arr = t_arr[:i]
            phi_arr = phi_arr[:i]
            beta_arr = beta_arr[:i]
            dcmd_arr = dcmd_arr[:i]
            dact_arr = dact_arr[:i]
            phase_arr = phase_arr[:i]
            break

    # Plot results
    fig, axes = plt.subplots(4, 1, figsize=(16, 14), sharex=True)

    phase_colors = {0:'white', 1:'#FFD700', 2:'#90EE90', 3:'#87CEEB', 4:'#DDA0DD'}
    
    # Shade phases
    for ax in axes:
        prev_p = phase_arr[0]
        start_t = t_arr[0]
        for i in range(1, len(phase_arr)):
            if phase_arr[i] != prev_p or i == len(phase_arr)-1:
                ax.axvspan(start_t*1000, t_arr[i]*1000, alpha=0.15,
                          color=phase_colors.get(prev_p, 'white'))
                prev_p = phase_arr[i]
                start_t = t_arr[i]

    axes[0].plot(t_arr*1000, np.degrees(phi_arr), 'b-', lw=1.5)
    axes[0].axhline(0, color='red', ls='--', alpha=0.5)
    axes[0].set_ylabel('phi [deg]', fontsize=12)
    axes[0].set_title(f'V15 Large Robot FWE: eps={FWE_EPS}, Tf={FWE_TF*1000:.0f}ms, '
                      f'Tw1={FWE_TW1*1000:.0f}ms | beta0=5d', fontsize=12)
    axes[0].grid(True, alpha=0.3)

    axes[1].plot(t_arr*1000, np.degrees(beta_arr), 'r-', lw=1.5)
    axes[1].axhline(0, color='black', ls='-', lw=0.5)
    axes[1].set_ylabel('beta [deg]', fontsize=12)
    axes[1].grid(True, alpha=0.3)

    axes[2].plot(t_arr*1000, np.degrees(dcmd_arr), 'g-', lw=1.5, label='cmd')
    axes[2].plot(t_arr*1000, np.degrees(dact_arr), 'k--', lw=1, alpha=0.5, label='actual')
    axes[2].set_ylabel('delta [deg]', fontsize=12)
    axes[2].legend(fontsize=9)
    axes[2].grid(True, alpha=0.3)

    axes[3].plot(t_arr*1000, phase_arr, 'k-', lw=1)
    axes[3].set_yticks(range(5))
    axes[3].set_yticklabels(['IDLE','FOLD','W1','EXT','W2'])
    axes[3].set_ylabel('Phase', fontsize=12)
    axes[3].set_xlabel('time [ms]', fontsize=12)
    axes[3].grid(True, alpha=0.3)

    plt.tight_layout()
    out = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'v10_large_convergence.png')
    plt.savefig(out, dpi=150)
    print(f"\nSaved convergence plot to: {out}")

    print("\n--- FWE Phase Log ---")
    for msg in fwe.phase_log:
        print(f"  {msg}")

    if len(phi_arr) > 0:
        print(f"\nFinal: phi={np.degrees(phi_arr[-1]):.2f}d beta={np.degrees(beta_arr[-1]):.2f}d")
    print(f"Cycles completed: {fwe.cycle_count}")
    print("Done!")


if __name__ == '__main__':
    main()
