"""Asymmetric FWE: fast FOLD + slow EXTEND (optimized speed)"""
import numpy as np
...
def run_asym(beta0, eps, T_fold, T_w, T_extend):
    ...
    af = 4*d_fold/T_fold**2
    ae = 4*d_fold/T_extend**2
    t1 = T_fold; t2 = t1+T_w; t3 = t2+T_extend

    def get_dd(t):
        if t < T_fold/2: return +af
        elif t < T_fold: return -af
        elif t < t2: return 0.0
        elif t < t2+T_extend/2: return -ae
        elif t < t3: return +ae
        else: return 0.0
    ...
    dt_max = min(T_fold, T_extend)/30
    sol = solve_ivp(rhs, (0, t3+0.001), [0,0,sigma0,0],
                    method='RK45', rtol=1e-10, atol=1e-12,
                    max_step=dt_max, dense_output=True)
    yf = sol.sol(t3)
    return yf[0], yf[2]/(Mt*h_com), sol, t3

beta0 = np.radians(5)

eps_vals = [3, 4, 5, 6]
tf_vals = [0.05, 0.08, 0.10]
te_vals = [0.20, 0.30, 0.50, 0.70, 1.00]
tw_range = np.linspace(0.01, 1.5, 100)  # coarser scan

print("Asymmetric FWE: fast fold + slow extend")
print("="*80)
...
for eps in eps_vals:
    ...
    for T_fold in tf_vals:
        for T_ext in te_vals:
            ...
            phis = []
            for tw in tw_range:
                pe, _, _, _ = run_asym(beta0, eps, T_fold, tw, T_ext)
                phis.append(pe)
            # Find zeros
            for k in range(len(phis)-1):
                if phis[k]*phis[k+1] < 0:
                    try:
                        tw_z = brentq(lambda tw: run_asym(beta0, eps, T_fold, tw, T_ext)[0],
                                     tw_range[k], tw_range[k+1], xtol=1e-6)
                        _, be, _, _ = run_asym(beta0, eps, T_fold, tw_z, T_ext)
                        r = be/beta0
                        if abs(r) < 1:
                            all_conv.append({
                                'eps':eps,'T_fold':T_fold,'T_w':tw_z,'T_ext':T_ext,
                                'r':r,'beta_f':be,'T_tot':T_fold+tw_z+T_ext,
                                'd_fold':d_deg,'v_fold':2*d_deg/T_fold,'v_ext':v_ext
                            })
                    except: pass
...
if all_conv:
    ...
    print(f"\n{'eps':>4} {'Tf':>5} {'Tw':>6} {'Te':>6} | {'Ttot':>5} | "
          f"{'r':>8} | {'d':>4} {'vf':>7} {'ve':>6}")
    print("-" * 75)
    for c in all_conv[:30]:
        print(f"{c['eps']:4} {c['T_fold']*1000:4.0f}ms {c['T_w']*1000:5.0f}ms "
              f"{c['T_ext']*1000:5.0f}ms | {c['T_tot']:.2f}s | "
              f"{c['r']:8.4f} | {c['d_fold']:3.0f}d {c['v_fold']:6.0f}d/s {c['v_ext']:5.0f}d/s")

    # Best trajectory
    best = all_conv[0]
    _, be, sol, t3 = run_asym(beta0, best['eps'], best['T_fold'], best['T_w'], best['T_ext'])
    Tf=best['T_fold']; Tw=best['T_w']; Te=best['T_ext']
    t1=Tf; t2=Tf+Tw; T_end=t3+0.05
    ...
    for t in t_plot:
        ...
        elif t<Tf: dt=t-Tf/2; delta_d.append(0.5*af*(Tf/2)**2+af*Tf/2*dt-0.5*af*dt**2)
        elif t<t2: delta_d.append(d_fold)
        elif t<t2+Te/2: dt=t-t2; delta_d.append(d_fold-0.5*ae*dt**2)
        elif t<t3: dt=t-(t2+Te/2); delta_d.append(d_fold-0.5*ae*(Te/2)**2-ae*Te/2*dt+0.5*ae*dt**2)
    ...
    axes[0].axvline(t3*1000, color='green', lw=2, label=f'EXTEND end')
    axes[0].set_ylabel('phi [deg]', fontsize=12); axes[0].legend(fontsize=11)
    ...
    axes[1].set_ylabel('beta [deg]', fontsize=12); axes[1].legend(fontsize=11)
    ...
    axes[0].set_title(
        f'Asymmetric FWE: eps={best["eps"]}, Tf={Tf*1000:.0f}ms, '
        f'Tw={Tw*1000:.0f}ms, Te={Te*1000:.0f}ms\n'
